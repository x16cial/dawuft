...
downloadLink.addEventListener('click', function () {
  // Construye la URL con los datos de las tareas como parámetros GET
  const url = 'descargar_tareas.php?' + tareas.map(tarea => `tarea[]=${encodeURIComponent(tarea.tarea)}&color[]=${encodeURIComponent(tarea.color)}`).join('&');

  // Redirige al navegador a la URL construida, forzando la descarga como .txt
  const a = document.createElement('a');
  a.href = url;
  a.download = 'lista_tareas.txt'; // Forzar la descarga como .txt
